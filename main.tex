...
\title{Operational predictability limits for urban PM10 and PM2.5 air-quality forecasting in Valencia and Madrid}
...
\begin{abstract}
Operational air-quality forecasting for public-health alerts requires
knowing not only whether a model outperforms persistence, but also up
to which horizon that advantage remains operationally useful across
consecutive days.
...
Valencia stations generally showed longer contiguous positive-skill
intervals than Madrid stations across pollutants and models,
consistent with the more stable meteorological regime of a coastal
Mediterranean setting.

These results establish quantitative predictability ceilings
for \ce{PM10} and \ce{PM_{2.5}} in two contrasting Spanish urban
environments and identify $\Hstrict$ as the operationally
relevant descriptor for warning lead times, city-specific forecast
usefulness, and deployment decisions in real air-quality alerting
contexts.
...
lead time, limiting its usefulness for alert systems, episode
...
A total of 46 city--station--pollutant series were
initially prepared for evaluation.
Three series were excluded because they did not provide
sufficient effective length for horizon-wise $\Hstar$
estimation: station~55 in Madrid (\ce{PM10}) and station
Olivereta in Valencia (\ce{PM10} and \ce{PM_{2.5}}).
The final dataset therefore comprised 43 independent
time series: 25 from Madrid (14~\ce{PM10},
11~\ce{PM_{2.5}}) and 18 from Valencia
(10~\ce{PM10}, 8~\ce{PM_{2.5}}).
...
This leakage-free design is especially important in urban air
quality series, where episodic concentration peaks can dominate
...
\caption{Operational predictability descriptors ($\Hrelax$, $\Hstrict$)
         ...
         ($H_{\max} = 7$~days). SP = simple persistence (baseline);
...
Under SARIMA, the median $\Hstrict$ was 2 days for \ce{PM10}
and 6 days for \ce{PM2.5}.
...
For \ce{PM_{2.5}}, the median $\Hstrict$ reached 6.5~days in
Valencia under both models, compared to 6~days in Madrid under
SARIMA and 5~days under XGBoost.
...
The finding that SARIMA achieved $\Hrelax = 7$~days in all evaluated
series indicates that linear models can retain some advantage over
simple persistence out to the maximum evaluated horizon. More broadly,
this is consistent with the persistence of synoptic-scale
meteorological patterns that shape particulate matter accumulation and
dispersion at weekly timescales over the Iberian Peninsula
\citep{Pey2013, Pay2014}. However, the systematic gap between
$\Hrelax$ and $\Hstrict$ shows that this advantage is often
discontinuous: models fall below persistence at one or more
intermediate horizons before recovering at longer lead times. For
operational forecasting, the relevant quantity is therefore
$\Hstrict$, not $\Hrelax$, because alert systems require reliable
guidance across consecutive days rather than isolated horizons.
...
43 evaluated series, confirming that the zero-skill reference is
correctly constructed and that the H* framework behaves as intended
under strict rolling-origin, train-only evaluation. This also
clarifies why persistence is a demanding baseline for daily
particulate matter forecasting: day-to-day autocorrelation is strong
enough that a model must capture genuine temporal structure beyond
lag-1 dependence to demonstrate positive skill \citep{Tashman2000}.
Studies that omit a persistence comparison therefore cannot establish
whether their forecasts provide operational added value for real
forecast use
\citep{GarciaCrespi2026}. The contribution here is therefore not to
identify a universal winning model, but to provide a reproducible
framework for evaluating operational usefulness over persistence.

\subsection{Fragmentation of skill curves: why H*\_strict matters}
\label{sec:disc_fragmentation}

The observation that $\Hstrict < \Hrelax$ in the majority of Madrid
series and in several Valencia series shows that skill curves are
often non-monotonic: positive skill at horizon $h$ does not guarantee
positive skill at $h+1$. This fragmentation is more pronounced for
\ce{PM10} than for \ce{PM_{2.5}}, and more pronounced in Madrid than
in Valencia, suggesting a stronger influence of episodic
high-concentration events such as Saharan dust intrusions and local
resuspension episodes \citep{Pey2013, Querol2024}. Aggregate error
metrics such as RMSE would obscure this structure, which is why
horizon-by-horizon skill assessment is essential for operational
interpretation \citep{Cerqueira2020}.

\subsection{Implications for operational air quality alert systems}
\label{sec:disc_operational}

These results have direct implications for the design and
communication of air quality alert systems in Spanish urban areas.
In Valencia, the high $\Hstrict$ values obtained under both
models—reaching 7 days for \ce{PM_{2.5}} at several stations—suggest
that a 7-day forecast window may be operationally defensible for fine
particulate matter, provided that deployment is benchmarked against
the strict rolling-origin, train-only protocol used here. In Madrid
for \ce{PM10}, the lower and more variable $\Hstrict$ values (1--6
days) indicate that forecast skill should be communicated more
cautiously beyond 2--3 days, especially at stations where XGBoost
collapses to $\Hrelax = 0$ (stations 4 and 36). In both cities, the
H* framework offers a transparent, station-specific basis for
communicating warning lead times and identifying operational windows
within which an advanced model adds value over persistence
\citep{GarciaCrespi2026}. This is directly relevant for city-level
alerting, where forecast usefulness must be interpreted at the
station level before deployment decisions are made.
...
First, both SARIMA and XGBoost retained positive skill over simple
persistence out to the maximum evaluated horizon of 7 days in the
large majority of stations, showing that useful forecasting beyond
the next day is achievable in many urban particulate matter series.
...
Median $\Hstrict$ ranged from 2 days (\ce{PM10}, Madrid, SARIMA)
to 6.5 days (\ce{PM_{2.5}}, Valencia, both models), with Valencia
consistently outperforming Madrid across pollutants and models.

Third, simple persistence produced $\Hrelax = \Hstrict = 0$ in all
43 evaluated series, validating the baseline construction and showing
that forecast performance reported without a persistence comparison
cannot be interpreted as genuine operational added value
\citep{GarciaCrespi2026}.

Fourth, the H* framework—combining $\Hrelax$, $\Hstrict$, and the
underlying horizon-by-horizon skill curve—provides a transparent and
station-specific basis for deciding whether an advanced model offers
sufficient operational value for deployment over persistence in
urban air-quality management. This makes the framework directly
useful for judging city-specific forecast usefulness before
operational deployment in these two Spanish urban monitoring contexts.
...
an AI-based language assistant for copy editing and
readability improvement of human-generated text. All content,
scientific claims, and numerical results are the sole
